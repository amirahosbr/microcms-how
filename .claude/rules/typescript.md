...
**Scope:** Apply when editing files matching `**/*.ts` or `**/*.tsx`.

- **Use path aliases for imports** - Use path aliases (e.g. `@lib/*`, `@utils/*`, `~~/shared/*`, `~~/app/*`) instead of relative imports (`../`). Only use `./` for same-directory imports.
